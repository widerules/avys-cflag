...
public class SaveThumbs implements Runnable {
	...
	public SaveThumbs(final Difficulty levelDclty, final int levelNo) {
		g = new Graphics();
		this.levelNo = levelNo;
		this.levelDclty = levelDclty;
	}

	public SaveThumbs(final Graphics g, final Difficulty levelDclty, final int levelNo) {
		this.g = g;
		this.levelNo = levelNo;
		this.levelDclty = levelDclty;
	}
	...
	public void setPlayAtlas() {
		g.setImageAtlas(g.createImageAtlas("play"));
	}

	public void disposePlayAtlas() {
		final TextureAtlas imageAtlas = g.getImageAtlas();
		if (imageAtlas != null) {
			imageAtlas.dispose();
		}
	}

	public void setLevelDclty(final Difficulty levelDclty) {
		this.levelDclty = levelDclty;
	}
	...
	public FileHandle createThumb() {
		final FileHandle fh = Gdx.files.external("Android/data/" + CFlagGame.packageName + "/thumbs/" + levelDclty.name() + levelNo + ".png");
		if (!fh.exists()) {
